...
import { getMultiYearSalesData, MultiYearSalesData, getFinancialMetrics, exportFinancialReport, FinancialMetrics, DateRange, getAvailableYears } from './actions';
...
export default function AnalyticsHub() {
  ...
  const [data, setData] = useState<MultiYearSalesData[]>([]);
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    // Since getAuditLogFeed is not implemented yet, handle the failure gracefully initially
    // or just assume we'll create it right after this.
    Promise.all([
      getMultiYearSalesData(stationId, selectedYears),
      getAuditLogFeed(stationId).catch(() => []) // Fallback until created
    ]).then(([res, auditRes]) => {
      setData(res.data);
      setYears(res.years);
